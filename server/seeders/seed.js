  ...
  const insertarDatos = db.transaction(() => {
    ...
    if (!adminExiste) {
      db.prepare(`
        ...
        VALUES (?, ?, ?, ?)
      `).run('Administrador', 'admin@adai.com', bcrypt.hashSync('admin123', 10), 'admin')

      db.prepare(`
        INSERT INTO usuarios (nombre, email, password_hash, rol)
        VALUES (?, ?, ?, ?)
      `).run('Cajero Principal', 'cajero@adai.com', bcrypt.hashSync('cajero123', 10), 'cajero')
    ...
    if (!prodExiste) {
      ...
      productos.forEach(p =>
        stmt.run(p.nombre, p.codigo, p.p_compra, p.p_venta, p.stock, p.stock_min, getCat(p.cat), p.iva)
      )
    ...
    if (!clienteExiste) {
      ...
      const stmt = db.prepare('INSERT INTO clientes (nombre, ruc_ci, telefono) VALUES (?, ?, ?)')
      clientes.forEach(c => stmt.run(c.nombre, c.ruc_ci, c.telefono))
    ...
    if (!provExiste) {
      ...
      const stmt = db.prepare('INSERT INTO proveedores (nombre, ruc, telefono) VALUES (?, ?, ?)')
      proveedores.forEach(p => stmt.run(p.nombre, p.ruc, p.telefono))
